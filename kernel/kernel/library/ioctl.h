...
NTSTATUS ctl_io(PDEVICE_OBJECT device_obj, PIRP irp)
{
	...
	if (stack_location)
	{
		...
		NOSCREEN_LOG("device control: code=0x%08X input=%lu output=%lu buffer=%p", ctl_code, input_buffer_length, output_buffer_length, system_buffer);
		switch (ctl_code)
		{
	...
	NOSCREEN_LOG("device control complete: status=0x%08X information=%llu", status, irp->IoStatus.Information);
