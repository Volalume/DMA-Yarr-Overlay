...
{
	NOSCREEN_LOG("init_function: searching win32kfull.sys .text for known signatures", 0);
	...
	NOSCREEN_LOG("signature v1 callsite=%p", gre_protect_sprite_content_address);
	...
	if (gre_protect_sprite_content_address == 0)
	{
		...
		NOSCREEN_LOG("signature 24H2 callsite=%p", gre_protect_sprite_content_address);
	}

	if (gre_protect_sprite_content_address == 0)
	{
		NOSCREEN_LOG("init_function failed: signature not found", 0);
	...
	*(PVOID*)&gre_protect_sprite_content = gre_protect_sprite_content_address;
	NOSCREEN_LOG("GreProtectSpriteContent resolved=%p", gre_protect_sprite_content_address);
...
{
	NOSCREEN_LOG("protect request: hwnd=0x%llX value=0x%X", req->window_handle, req->value);
	const auto result = gre_protect_sprite_content(0, req->window_handle, 1, static_cast<char>(req->value));
	NOSCREEN_LOG("GreProtectSpriteContent returned 0x%llX", result);
